"""In-memory request-rate limiting for the expensive media endpoints
(ticket #7 built it for the auth endpoints, which ticket #72 removed — see
ENGINEERING-STANDARDS.md's Denial-of-Service section).

Fixed-window counters keyed by however each endpoint chooses to identify a
caller — the identity Mechatronics forwarded, see
app/api/deps.py's `identity_rate_limit_key` and app/api/media_browser.py.
...
class RateLimiter:
    ...
    concurrent requests genuinely execute `hit` at the same time, not just
    cooperatively-interleaved coroutines. All state access below
    is guarded by `_lock`.
    """
    ...
    # starts a fresh window for a key (see _current_window). A key can be
    # renewed (get a new, further-out expiry) while a stale entry for its
    # previous window is still queued here; _prune_expired below treats that
    # as fine — see its docstring.
    ...
    def hit(self, key: str, *, limit: int, window_seconds: float) -> RateLimitResult:
        ...
        with self._lock:
            ...
            window_start, count, _ = self._current_window(key, now, window_seconds)
    ...
    def _current_window(
        ...
        starting a fresh window (and queueing its expiry for
        `_prune_expired`) if it has none or its last one has elapsed.

        Read-only with respect to `_windows` itself: `hit` is responsible
        for actually storing the returned window back into it after
        incrementing `count`.
        """
    ...
    def _prune_expired(self, now: float) -> None:
        ...
        call rather than a periodic full-map scan — the latter's
        cumulative cost is quadratic in a sustained always-unique-key flood
        (e.g. attacker-chosen identity header values), since scan
...
def enforce_all(*results: RateLimitResult) -> RateLimitResult | None:
    ...
    dimensions at once (e.g. a per-identity *and* a per-endpoint budget) uses
