...
def test_navigating_into_a_dataset_shows_its_immediate_children(client, db_session, s3_client):
    ...
    response = client.get("/media/datasets", params={"path": "dataset_v0.9"}, headers=headers)
...
def test_browsing_an_unknown_dataset_path_returns_not_found(client, db_session, s3_client):
    ...
    response = client.get(
        "/media/datasets", params={"path": "dataset_v0.9/does-not-exist"}, headers=headers
    )
...
def test_user_and_admin_have_identical_dataset_access(client, db_session, s3_client):
    ...
    user_response = client.get(
        "/media/datasets", params={"path": "dataset_v0.9"}, headers=user_headers
    )
    admin_response = client.get(
        "/media/datasets", params={"path": "dataset_v0.9"}, headers=admin_headers
    )
